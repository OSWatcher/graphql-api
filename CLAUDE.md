...
- All resolvers receive `jwt` and `permissions` from Auth0 context
...
- Integrates authorization check before serving blobs

**Authorization (`src/blob-authorization.ts`)**
- `isBlobRestricted()` checks if blob belongs only to restricted branches
- Uses `CHECK_BLOB_RESTRICTED_QUERY` Cypher query from `src/queries.ts`
- Fails safe: treats unknown/error cases as restricted
...
- **Auth0**: `AUTH0_DOMAIN_URI`, `AUTH0_AUDIENCE`
...
### Security Patterns
- JWT validation via Auth0 middleware (`checkJwt` with `authRequired: false`)
- Context injection: `jwt` and `permissions` available in all resolvers
- Sensitive data filtering happens at response time, not query time
- Fail-safe approach: default to restrictive on errors
