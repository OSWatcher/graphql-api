...
const GitSHA1Schema = z.string().regex(/^[a-f0-9]{40}$/i, "Invalid commit hash format");

// Commit scope enum - map to OGM enum
const CommitScopeSchema = z.enum(["SINGLE", "HISTORY", "HISTORY_WITH_UPDATES", "RANGE"])
    .transform((val) => {
        switch (val) {
            case "SINGLE": return CommitScope.Single;
            case "HISTORY": return CommitScope.History;
            case "HISTORY_WITH_UPDATES": return CommitScope.HistoryWithUpdates;
            case "RANGE": return CommitScope.Range;
            default: return val;
        }
    });
